...
For example, in the Cairo VM we have 15 flags. This flags include  "DstReg", "Op0Reg", "OpCode" and others. For simplification, let's imagine with hae 3 flags them with letters from "A" to "C", where "A" is the first flag. 
...
A Virtual Column is like a traditional column, which is own set of constraints, but it exists interleaved with another one. In the previous example, each row is associated with a column, but in practice, we could have different ratios. We could have 3 rows corresponding to one Virtual Column, and the next oen corresponding to another one. For the time being, let's focus on this simpler example.
...
Now, to do this, we will need to evaluate the multiple rows taking in account they are part of the same step. For a real case, we will add a dummy flag D, whose purpose is to make the evaluation move in a number which is a potency of 2. 
...
Assume now we have 3 columns that share some constraint. For example, let's have three flags that can be either 0 or 1. Each flag will also have it's own set of dedicated constraints. 
