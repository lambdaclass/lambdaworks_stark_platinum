...
# Commitments
During the protocol several polynomials are *commited*. In this context this means the following. There will be a fixed vector called $D = (d_1, \dots, d_l)$. To commit to a polynomial $f$, the prover evaluates $f$ at every point of $D$ to obtain the vector $(f(d_1), \dots, (f(d_l)))$. Then the prover builds a Merkle tree out of it and sends the root of the tree to the verifier.

The verifier can then ask the prover to *open* $f$ at some points of $D$ and the prover won't have any choice other than send the correct values. This is because the verifier will expect not only the corresponding values $f(d_i)$, but also authentication paths to the root of the tree to check their authenticity.

Note that the prover can only open $f$ at points that belong to $D$. So, more formally, the prover actually commited to the vector $(f(d_1), \dots, (f(d_l)))$. The fact that this vector is the vector of evaluations of a polynomial $f$ at $D$ needs a separate proof. And that's where the FRI protocol enters. This is run at the end of the STARKs protocol. So the verifier receives a bunch of vector commitments and only at the very end he will be convinced that these vectors are actually evaluations at $D$ of polynomials of low degree. Until then, they are just any vector commitments potentially unrelated to anything.
